...
const _sfc_main$h = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const updateInternals = () => {
      ...
      const currentExtent = applyExtent(node.value, nodeExtent.value, parentNode.value);
      const nextPos = currentExtent && typeof currentExtent !== "string" ? clampPosition(node.value.position, currentExtent) : node.value.position;
      updatePosition2(
        {
          ...nextPos,
      ...
      scope.run(() => {
        ...
        watch(
          ...
          ([newX, newY, parentX, parentY, parentZ]) => {
            const currentExtent = applyExtent(node.value, nodeExtent.value, parentNode.value);
            const nextPos = currentExtent && typeof currentExtent !== "string" ? clampPosition({ x: newX, y: newY }, currentExtent) : { x: newX, y: newY };
            const xyzPos = {
              ...nextPos,
